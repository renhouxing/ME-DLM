...
def mathd_normalize_answer(answer: str):
    if answer is None:
        return None
    answer = answer.strip()
    try:
...
def math_reward_fn(generation, answers):
    ...
    if isinstance(answers, (str, float, int)):
        answers = [answers]
    ...
    for answer in answers:
        is_correct = grade_answer_mathd(model_answer, answer) or grade_answer_sympy(model_answer, answer)
        if is_correct:
            break
